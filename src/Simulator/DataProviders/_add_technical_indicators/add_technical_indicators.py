...
def add_technical_indicators(df, interval = None, **params):
    ...
    for indicator, period in technical_indicators.items():
        ...
        if indicator == 'bollinger_band':
            if f'stat_bollinger_bands_upper{suffix}' in df.columns:
                continue
            bb = ta.bbands(df['Close'], length=period)
            df[f'stat_bollinger_bands_upper{suffix}'] = bb.iloc[:, 0].shift()
            df[f'stat_bollinger_bands_lower{suffix}'] = bb.iloc[:, 2].shift()

            df[f'stat_open_over_lower_BB_ratio{suffix}'] = df['Open'] / \
                df[f'stat_bollinger_bands_lower{suffix}']
