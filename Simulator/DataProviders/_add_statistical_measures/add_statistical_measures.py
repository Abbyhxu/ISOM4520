...
def add_statistical_measures(
    ...
    def _calculate_beta_(df_close, df_market):
        # Calculate covariance between stock and market
        covariance = np.cov(df_close, df_market)
    ...
    if macro_and_other_data is not None:
        ...
        for k, data in macro_and_other_data.items():
            ...
            if f'stat_{k}_change(t_1)_ratio{suffix}' not in df:
                df[f'stat_{k}_change(t_1)_ratio{suffix}'] = data['Close'].pct_change(
                ).shift()

            # adding original market macro and market index data (without pct change)
            if f'stat_{k}{suffix}' not in df:
                df[f'stat_{k}{suffix}'] = data['Close'].pct_change(
                ).shift()

    for t in [22]:
        # adding iv
        if f"stat_Vola({t}){suffix}" not in df:
            df[f"stat_Vola({t}){suffix}"] = df['Close'].shift(
            ).pct_change().rolling(window=t).std()*(252**0.5)

        # adding market beta
        if f"stat_Beta({t}){suffix}" not in df:
            df[f"stat_Beta({t}){suffix}"] = df.shift().pct_change().rolling(window=t).cov().unstack()[
                'Close'][f'stat_market_index_change(t_1)_ratio{suffix}']/df['Close'].shift().pct_change().rolling(window=t).var()
